...
Ext.onReady(function(){
    ...
    function extractPermalink(language){
        ...
        return Ext.ux.services.get("base") + "?lat=" + c.lat + "&lon=" + c.lng + "&zoom=" + z + mLatLng + startHash + destHash + viaHash + lang;
    }
    ...
    function doRouting(recenter){
        ...
        if(startmarker && endmarker) {
            ...
            Ext.Ajax.request({
                ...
                url: Ext.ux.services.get("directions")
    ...
    externalLinks.on('click', function(event, htmlElement, o){
        ...
        htmlElement.href = Ext.ux.services.get("redirect") + "?u=" + escape(originalHref);
    });
...
Ext.ux.LanguageButton = Ext.extend(Ext.Button, {
    ...
    initComponent: function(){
        ...
        var config = {
            enableToggle: true,
            handler: function(evt) {
                Ext.util.Cookies.set('__LANG__', this.languageCode, new Date().add(Date.DAY, 30));
        ...
        var config = {
            ...
            store: new Ext.data.JsonStore({
                proxy : new Ext.data.HttpProxy({
                    method: 'GET',
                    url: Ext.ux.services.get("places")
                }),
                root: 'data',
                idProperty: 'hash',
...
Ext.ux.MapPanel = Ext.extend(Ext.Panel, {
    ...
    updateCookie: function(){
        ...
        + this.map.getZoom();
        Ext.util.Cookies.set('__LOCATION__', value, new Date().add(Date.DAY, 30), Ext.ux.services.get("base"));
    }
});
