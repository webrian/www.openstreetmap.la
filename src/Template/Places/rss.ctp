...
<rss version="2.0" 
    ...
    <channel>
        ...
        <link><?php echo $this->Url->build("/places/rss?q=$searchTerm", true); ?></link>
        ...
        <atom:link rel="search" type="application/opensearchdescription+xml"
                   href="<?php echo $this->Url->build("/places.xml", true); ?>"/>
        ...
        foreach ($result['data'] as $item) {
            ...
            $link = $this->Url->build("/?mlon=" . $item['lon'] . "&mlat=" . $item['lat'] . "&zoom=14", true);
            echo "<link>$link</link>";
